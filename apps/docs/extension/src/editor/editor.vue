...
<template>
  <div ref="editor" class="ext-panel" :class="{ moving }" :style="{ top: `${top}px`, left: `${left}px` }">
    <img v-show="!showSidebar" class="entrance touch-icon" src="./icon-entrance.png" @click="toggleSidebar">
    <img v-show="showSidebar" class="close touch-icon" src="./icon-close.png" @click="toggleSidebar">
    <div v-show="showSidebar" class="editor" :style="{ height: `${height}px` }">
      <el-tabs v-model="activeTab" @tab-click="onTabClick">
        <el-tab-pane label="Config" name="config">
          <ThemeConfigurator
            :themeConfig="themeConfig" :previewConfig="previewConfig"
            :onUserConfigUpdate="onUserConfigUpdate" from="extension"
          />
        </el-tab-pane>
        <el-tab-pane label="Gallery" name="gallery">
          <Gallery ref="gallery" :height="height" :width="width - 7" @action="onGalleryAction" />
        </el-tab-pane>
      </el-tabs>
    </div>
  </div>
