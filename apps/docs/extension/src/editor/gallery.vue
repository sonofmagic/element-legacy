...
<template>
  ...
  >
    ...
    >
      <li v-for="item in userTheme" :key="item.name" class="theme-card">
        <ThemeCard type="user" :config="item" from="extension" @action="onAction" />
      </li>
      <li class="theme-card">
        <ThemeCard type="upload" :config="{ name: 'upload' }" @action="onAction" />
      </li>
    </ul>
    <el-dialog :visible.sync="copyDialogVisible" :modal-append-to-body="false">
      <el-form ref="copyForm" :model="copyForm" :rules="copyFormRule">
        <el-form-item label="主题名称" prop="name">
          <el-input v-model="copyForm.name" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="closeCopyForm">
          取消
        </el-button>
        <el-button type="primary" @click="copyToUser">
          确认
        </el-button>
      </div>
    </el-dialog>
