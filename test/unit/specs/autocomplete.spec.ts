...
type Restaurant = Record<string, string>
...
describe('autocomplete', () => {
  ...
  it('create', (done) => {
    vm = createVue({
      ...
      methods: {
        querySearch(this: any, queryString: string, cb: (results: Restaurant[]) => void) {
          let restaurants = this.restaurants
          let results = queryString ? restaurants.filter(this.createFilter(queryString)) : restaurants
          cb(results)
        },
        createFilter(this: any, queryString: string) {
    ...
    const createVm = (selectWhenUnmatched = false) => {
      return createVue({
        ...
        methods: {
          ...
          handleSelect(this: any, item: Restaurant) {
            this.item = item
          },
        },
        mounted(this: any) {
    ...
    vm = createVue({
      ...
      methods: {
        querySearch(this: any, queryString: string, cb: (results: Restaurant[]) => void) {
          ...
          cb(
            queryString
              ? opts.filter(opt => opt.value.includes(queryString))
              : opts,
          )
        },
      },
