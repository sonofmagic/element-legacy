...
export default {
  ...
  computed: {
    navStyle() {
      const dir = ['top', 'bottom'].includes(this.rootTabs.tabPosition) ? 'X' : 'Y'
      return {
        transform: `translate${dir}(-${this.navOffset}px)`,
      }
    },
    sizeName() {
      return ['top', 'bottom'].includes(this.rootTabs.tabPosition) ? 'width' : 'height'
    },
  },
  ...
  methods: {
    ...
    changeTab(e) {
      ...
      if ([37, 38, 39, 40].includes(keyCode)) { // 左右上下键更换tab
    ...
    return (
      <div class={['el-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
        ...
        <div class={['el-tabs__nav-scroll']} ref="navScroll">
          <div
            class={['el-tabs__nav', `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].includes(this.rootTabs.tabPosition) ? 'is-stretch' : '']}
            ref="nav"
            style={navStyle}
            role="tablist"
