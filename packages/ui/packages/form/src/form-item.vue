...
export default {
  ...
  methods: {
    validate(trigger, callback = noop, validateOptions) {
      ...
      const options = this.getValidateOptions(validateOptions)

      validator.validate(model, options, (errors, invalidFields) => {
    ...
    getValidateOptions(options) {
      const mergedOptions = objectAssign({ firstFields: true }, this.form && this.form.validateOptions ? this.form.validateOptions : {})

      return objectAssign(mergedOptions, options || {})
    },
    clearValidate() {
      this.validateState = ''
